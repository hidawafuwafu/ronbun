...
全日本コーヒー協会の統計資料「日本のコーヒー消費と株価」では，2018年のコーヒー消費量がおよそ470千トンを記録していることがみてとれる\cite{Railshon1}．
...
コーヒーノキ（アラビカ種）はエチオピアのアビシニア高原にて発見された．その後，アラビアに伝播しオランダの貿易商人達の手によってアフリカやアジアへと広がっていき，商業用生産が活発になる．UCCホームページより，コーヒーの軌跡を図\ref{fig:kiseki}に示す．
...
\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=100mm]{figure/map.pdf}
 \end{center}
...
\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=80mm]{figure/c.pdf}
 \end{center}
  \caption{クレバードリッパー}
  \label{fig:cl}
\end{figure}
...
\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=80mm]{figure/a1.pdf}
 \end{center}
  \caption{エアロプレス}
  \label{fig:aero}
\end{figure}
...
UCCは，コーヒーの抽出に限らず，豆の選び方や焙煎の種類など，他の企業に比べ包括的で，辞典のようなWebページを掲載している．
抽出に関しては，抽出手順が図解されているほか，動画で確認をすることができる．また，他社に比べて理論的な説明が多くみられる．
STARBUCKSは，自社のコーヒー豆の紹介をメインにしており，抽出に関しては，複数の器具の説明を図と文字でしている．
TULLY’Sは，他社と同様に図解で抽出方法を説明しているほか，自社ページ以外にもYouTubeで美味しい淹れ方の動画を掲載している．
...
河原（2018）の，音声認識の方法論の変遷を表として，表\ref{tb:onsei}に示す．
この表1を見ると現在の音声認識の方法論は4.5世代にあたり，ニューラルネットワークの利用が主流になっていることがわかる．
ニューラルネットワークはいわば機械学習の類であり，それらを応用することで音声認識を成し遂げている．
実際に，Google社の手がける音声認識サービス「Google Cloud Speech API」においてもニューラルネットワークモデルが利用されていることが明記されている\cite{Railshon2}．
...
「レシピ閲覧画面」では，実際に器具ごとのコーヒー抽出レシピを閲覧できる．
実際のレシピ閲覧画面を図\ref{fig:step1}，および図ref{fig:recipieturan}に示す．
...
\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=130mm]{figure/soturon3.pdf}
 \end{center}
  \caption{レシピ閲覧画面：器具の確認}
...
\subsubsection{\large{音声認識の実装について}}
音声認識の実装にはJavaScriptを使用した．
宗定(2014)の記事を元に作成した「seramic.js」\cite{Railshon3},
